...
1. ブラウザで VSCode にアクセス → [環境セットアップガイド](docs/reference/ENVIRONMENT_SETUP.md)
...
│   └── reference/               # リファレンス資料（環境構築, Tips, FAQ, 各種ガイド）
├── evaluation/                  # 評価チェックリスト
├── scripts/
│   ├── check.sh                 # セッション完了チェックスクリプト
│   ├── break_session.sh         # 障害シミュレーション（セッション4用）
│   └── break_session9.sh        # 障害シミュレーション（セッション9用）
