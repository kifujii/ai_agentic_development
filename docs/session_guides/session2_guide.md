...
>
> 💡 **Agentのターミナル操作について**: このセッションでは、AgentがSSHでEC2に接続してコマンドを実行する場面があります。AgentはContinueのターミナル機能を使って **自動的にSSH接続→コマンド実行→結果確認** を行います。VS Codeのターミナルに操作が表示されるので観察してみてください。
