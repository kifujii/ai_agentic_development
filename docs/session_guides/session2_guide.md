...
EC2 がブラウザからの HTTP アクセスを受け付けられる状態。セキュリティグループに HTTP(80) ルールが追加され、次の Step で nginx をインストールすればすぐ Web ページが公開できる。
...
> **💡 Agentのターミナル操作について**
>
> このStepでは、Agentが**ターミナルを使ってリモートサーバー（EC2）にSSH接続し、コマンドを実行**します。セッション1ではローカルのファイル生成と `terraform` コマンドの実行が中心でしたが、ここからは「Agent がSSHでサーバーに入り、パッケージのインストールやサービスの起動を行う」という質的に異なる操作になります。
>
> Agentがターミナルでコマンドを実行する際は、**実行前に承認を求められます**。SSH接続コマンドや `sudo` を含むコマンドが表示されたら、内容を確認してから承認してください。何をしようとしているのか理解した上で進めることが重要です。
...
EC2 上で nginx が起動し、サーバー再起動後も自動的に立ち上がる状態。この時点ではデフォルトの Welcome ページが表示される。
...
ブラウザでアクセスするとオリジナルのWebページが表示される状態。nginx のデフォルトページではなく、自分で指示した内容のカスタムページが公開されている。
...
改善されたWebページが EC2 上にデプロイされ、ブラウザで確認できる状態。「修正 → デプロイ → 確認」のサイクルを体験し、繰り返し改善できる流れを理解している。
