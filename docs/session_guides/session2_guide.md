# セッション2：Terraform でインフラを構築・変更・再構築しよう（必須・1時間）
...
Step 1: セキュリティグループに HTTP(80) を追加（5分）
    ↓
Step 2: SSH で nginx をインストール・確認（10分）
    ↓
Step 3: terraform plan でインフラを変更してみよう（10分）
    ↓
Step 4: terraform destroy で全リソースを削除（5分）
    ↓
Step 5: user_data で自動化して一発再構築（15分）
    ↓
...
## Step 2: SSH で nginx をインストールしよう（10分）
...
## Step 5: user_data で自動化して一発再構築しよう（15分）
