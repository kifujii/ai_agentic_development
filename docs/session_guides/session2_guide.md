...
>
> 💡 **Clineのターミナル操作について**: このセッションでは、ClineがSSHでEC2に接続してコマンドを実行する場面があります。Clineはターミナル機能を使って **自動的にSSH接続→コマンド実行→結果確認** を行います。各コマンド実行前に承認を求められるので、内容を確認してからAcceptしてください。
