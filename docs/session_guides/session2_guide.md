...
```
terraform/vpc-ec2/ のセキュリティグループに HTTP(80) アクセスを追加して、
Webサーバーを外部公開できるようにしてください。
...
```
EC2 に SSH で接続して nginx をインストール・起動してください。
■ 接続情報
- IP: <EC2のIPアドレス>
...
```
terraform/vpc-ec2/ の EC2 定義に user_data を追加して、
EC2 起動時に nginx が自動インストール・起動されるようにしてください。
カスタムの index.html も自動配置してください。
変更後、terraform apply で全リソースを再構築してください。
```
...
```
EC2上の /usr/share/nginx/html/index.html を以下の要件で改善してください。
