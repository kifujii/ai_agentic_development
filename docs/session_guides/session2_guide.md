...
1. Agentの実行結果で `active (running)` が表示されていること
2. ブラウザで `http://<EC2のIPアドレス>` にアクセスして **nginx のデフォルトページ** が表示されること
...
> 💡 **この手動インストールを覚えておいてください。** Step 5 では、同じことを Terraform の `user_data` で自動化します。
...
2. セキュリティグループの description を "Training workshop security group" に変更
...
Agentが `terraform plan` を実行すると、以下のような差分が表示されます：
...
plan の内容を確認したら、Agentに `terraform apply` を実行してもらいましょう。
...
1. `terraform destroy` の出力で `Destroy complete! Resources: N destroyed.` が表示される
2. AWS コンソールで確認（任意）:
   - **VPC** → 対象VPCが消えている
...
`terraform apply` 完了後、新しいIPアドレスを確認します：
...
EC2の `user_data` が実行完了するまで **1〜2分** 待ってから、ブラウザで `http://<新しいIPアドレス>` にアクセスします。
...
| カスタムページの改善・デプロイ | Agent を使った反復的な改善サイクル |
