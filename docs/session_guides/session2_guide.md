# セッション2：Terraform でインフラを構築・変更・再構築しよう（必須・45分）

## 学習目標

- Terraformのライフサイクル（plan/apply/destroy）を一通り体験する
- IaCの真価（コードによる再現性と自動化）を実感する
...
```
Step 1: セキュリティグループに HTTP(80) を追加（5分）
    ↓
Step 2: SSH で nginx をインストール・確認（5分）
    ↓
Step 3: terraform plan でインフラを変更してみよう（5分）
    ↓
Step 4: terraform destroy で全リソースを削除（5分）
    ↓
Step 5: user_data で自動化して一発再構築（10分）
    ↓
Step 6: ブラウザで確認 + カスタムページのデプロイ（10分）
    ↓
振り返り（5分）
```
...
## Step 1: セキュリティグループに HTTP を追加しよう（5分）
...
- セキュリティグループに **HTTP アクセスの許可** が追加されている
...
```
terraform/vpc-ec2/ のセキュリティグループに、ブラウザからWebページにアクセスできるようルールを追加してください。
既存の SSH ルールはそのまま残してください。
...
## Step 2: SSH で nginx をインストールしよう（5分）
...
既存リソースに何か変更を加えて、`terraform plan` で差分がどう表示されるか確認してみましょう。変更内容は自由ですが、例えば以下のようなものがあります：
...
```
terraform/vpc-ec2/ の既存リソースにいくつかタグを追加してください。
何を追加するかは任せます。
...
AWSコンソールでタグの変更が反映されていれば OK ✅

---

## Step 4: terraform destroy で全リソースを削除しよう（5分）
...
## Step 5: user_data で自動化して一発再構築しよう（10分）
...
EC2 が起動したら自動的に nginx がインストール・起動され、
カスタムの Web ページが表示されるようにしてください。
...
## Step 6: カスタムWebページを改善・再デプロイしよう（10分）
...
どんなページにするかは自由です。Claude Code に「こんなページにしたい」と伝えて、自分だけの Web ページを作ってみましょう。

<details>
<summary>📝 プロンプト例</summary>

```
EC2上のWebページをもっとかっこよくしてください。
デザインは任せます。
...
> 💡 カスタムWebページは EC2 上の `/usr/share/nginx/html/index.html` に直接配置されています。
...
- **任意課題に挑戦**: [セッション3：Webサーバーを冗長構成にしよう](session3_guide.md)
