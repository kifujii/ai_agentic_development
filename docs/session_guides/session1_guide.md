...
EC2 に SSH でログインするための準備が整った状態。Terraform で SSH 鍵の登録とファイアウォール（22番ポートのみ許可）が構成され、次の Step で EC2 を作ればすぐ接続できる。

### やること

- `terraform/vpc-ec2/` に **キーペア** と **セキュリティグループ** を追加して apply
- キーペアは事前準備で作った `keys/training-key.pub` を使う
- セキュリティグループはSSH（22番ポート）だけ通す
- outputs.tf にセキュリティグループIDを **`security_group_id`** という名前で出力する
...
インターネットからSSHでアクセス可能なEC2インスタンスが稼働している状態。パブリックIPが割り当てられ、次の Step で実際にSSH接続して動作確認できる。

### やること

- `terraform/vpc-ec2/` にEC2インスタンスを追加して apply
- Amazon Linux 2023、t3.micro
- Step 2 のパブリックサブネットに配置
- Step 3 のキーペアとセキュリティグループを使用
- outputs.tf にパブリックIPを **`instance_public_ip`** という名前で出力する
- outputs.tf にインスタンスIDを **`instance_id`** という名前で出力する
