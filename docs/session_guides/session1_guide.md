...
> ⚠️ **Terraform コードの保存先**: このセッションで作成するすべての Terraform コードは **`terraform/vpc-ec2/`** ディレクトリに保存してください。完了チェック（`check.sh`）はこのパスを前提にしています。
...
- outputs.tf に VPC ID を出力（output名: vpc_id）
...
> ⚠️ **output名について**: 後続のセッションや完了チェックでこの output を参照します。output 名は必ず `vpc_id` にしてください。
...
- `terraform output subnet_id` でサブネットIDが表示される
...
- インターネットゲートウェイ
- ルートテーブル（0.0.0.0/0 → IGW）
- outputs.tf にサブネットIDを追加（output名: subnet_id）
...
> ⚠️ **output名について**: 後続のセッションや完了チェックでこの output を参照します。output 名は必ず `subnet_id` にしてください。
...
> **ワークショップ期間中はリソースを削除しないでください。** ここで作成した EC2 は後続セッション（セッション2〜6）でも使用します。
>
