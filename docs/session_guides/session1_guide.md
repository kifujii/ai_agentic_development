...
> - 「Claude Code が〜」「Agent が〜」 → Claude Code が自動的に行う処理です
...
> 💡 **Agent開発の特徴**: あなたがプロンプトを入力するだけで、Claude Code が「コード生成→ファイル保存→terraform init→apply」まで自動で進めてくれます。
...
- エラーが出たら、**エラーメッセージをそのまま Claude Code に伝えて**ください — Claude Code が自動修正してくれます
...
> 2. Claude Code が実行し、あなたが結果を確認する（成功 or エラー）
> 3. エラーの場合 → エラーメッセージをそのまま Claude Code に共有
> 4. Claude Code が原因を分析し、修正案を提示・実行
> 5. あなたが結果を再確認
> 6. 解決するまで 3〜5 を繰り返す
> ```
>
> 💡 **ポイント**: Claude Code はエラーメッセージから多くの情報を読み取れます。「動きません」よりも **エラーの全文を貼り付ける** 方が、はるかに正確な診断ができます。
>
...
| 特徴 | 体験したこと |
|------|------------|
| **コード生成→実行の自動化** | あなたがプロンプトを入力するだけで、Claude Code が terraform init → apply まで自動実行 |
| **段階的な構築** | Claude Code に「既存コードに追加」と指示するだけで段階的に構築できた |
| **承認ワークフロー** | Claude Code が提示する変更内容を、あなたが確認してから実行（human in the loop） |
| **エラー自動修正** | エラー発生時、Claude Code が自動で原因を分析し修正を提案 |
