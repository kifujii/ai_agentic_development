...
## Step 2: サブネットとインターネット接続を追加しよう — 🟡 ガイド付き（20分）
...
## Step 3: キーペアとセキュリティグループを作ろう — 🟠 ヒント付き（15分）
...
- キーペアは事前準備で作った `~/.ssh/training-key.pub` を使う
- セキュリティグループはSSH（22番ポート）だけ通す

### 💡 プロンプトのヒント

- キーペア名は `"training-key"` にしましょう
- セキュリティグループ名は `"training-ec2-sg"` にしましょう
- アウトバウンドは全許可が一般的です
- outputs にセキュリティグループIDを追加しましょう
...
## Step 4: EC2インスタンスを作ろう — 🔴 自力で挑戦！（20分）
