# セッション1：VPC + EC2 を段階的に構築しよう（必須・1.5時間）
...
```
Step 0: Plan モードで設計を相談     ← AI と対話しながらインフラ設計    (15分)
    ↓
Step 1: VPC を作る                  ← お手本プロンプトで体験            (20分)
    ↓
Step 2: ネットワーク環境を整える     ← 目的を伝えてプロンプトを考える    (20分)
    ↓
Step 3: SSH の準備をする             ← ゴールだけ示してヒントで挑戦      (15分)
    ↓
Step 4: EC2 を作る                   ← 自力で挑戦！                     (15分)
    ↓
...
1. ターミナルで Plan モードで Claude Code を起動します：

```bash
claude --plan
```

> 💡 既に Claude Code を起動中の場合は、`Shift + Tab` で Plan モードに切り替えることもできます。
...
## Step 2: ネットワーク環境を整えよう — 🟡 ガイド付き（20分）
...
## Step 4: EC2インスタンスを作ろう — 🔴 自力で挑戦！（15分）
