...
> このセッション以降、何か問題が起きたときは以下のパターンで対応できます。
>
> ```
> 1. AI にタスクを依頼する
> 2. 実行結果を確認する（成功 or エラー）
> 3. エラーの場合 → エラーメッセージをそのまま AI に共有
> 4. AI が原因を分析し、修正案を提示・実行
> 5. 結果を再確認
> 6. 解決するまで 3〜5 を繰り返す
> ```
>
> 💡 **ポイント**: AI はエラーメッセージから多くの情報を読み取れます。「動きません」よりも **エラーの全文を貼り付ける** 方が、はるかに正確な診断ができます。
>
> このパターンは Ansible に限らず、Terraform、AWS CLI、どんなツールでも同じです。**Step 6 の障害対応シミュレーション** でこのパターンを実際に体験します。
...
まず、nginx を停止し、設定ファイルを壊します。以下のコマンドを **1行ずつ** 実行してください：
...
ブラウザで `http://<EC2のIP>` にアクセスしてみてください。**ページが表示されない**（接続がタイムアウトする）ことを確認します。

---

### Phase 2: Claude Code にトラブルシューティングを依頼する（15分）

障害の状況だけを Claude Code に伝えて、原因調査から復旧まで一貫して対応してもらいます。
...
ブラウザで `http://<EC2のIP>` にアクセスして、ページが表示されれば **復旧成功** ✅

---

### Phase 3: 復旧手順を Playbook 化する（10分）

同じ障害が将来起きたときにすぐ対応できるよう、**nginx の診断・復旧 Playbook** を作成しましょう。
...
| 体験 | 学んだこと |
|------|-----------|
| nginx を壊して復旧 | AIは「確認 → 起動失敗 → 原因特定 → 修復 → 再起動 → 確認」を自動で繰り返す |
| エラーメッセージを AI に共有 | **具体的なエラー情報が、正確な診断につながる** |
| 手動復旧 → Playbook 化 | **一度手動で対応したら、次回のために自動化する** |

> 🎓 **このパターンを覚えておきましょう**: 何か問題が起きたら → **エラーメッセージを Claude Code に共有** → AI が原因を分析・修正。このパターンはセッション5以降でも使えます。
...
> 💡 セッション5で問題が起きた場合も、このセッションで学んだ「エラーメッセージを AI に共有 → 原因分析 → 修正」のパターンで対応できます。
