...
Ansible から EC2 に接続するための設定が完了している状態。`ansible/` フォルダに設定ファイルが揃い、次の Step で接続テストを行える。
...
- EC2が起動しているか確認
- IPアドレスが正しいか確認（`cd terraform/vpc-ec2 && terraform output instance_public_ip && cd ../..`）
...
Playbook を実行するとサーバーの現在の状態（OS、メモリ、ディスク等）が一覧表示される状態。運用でサーバーの健全性を素早く確認できるツールが手に入る。
...
サーバーを安全に再起動でき、再起動前後の状態変化を確認できる運用 Playbook がある状態。サービスの正常性も含めて自動チェックされるので、再起動による問題を即座に検知できる。
...
任意のサービスを起動・停止・再起動できる汎用的な Playbook がある状態。変数を変えるだけで対象サービスとアクションを切り替えられるので、1つの Playbook で様々なサービスを管理できる。
...
nginx の状態確認・安全な再起動・ヘルスチェックまでを一括で行えるメンテナンス用 Playbook がある状態。設定変更後のリスタートも、Web サイトの応答確認まで自動化されている。
