# セッション4：Ansible によるサーバー運用自動化（必須・1.5時間）
...
Step 2: 接続テスト - ping（5分）
    ↓
Step 3: サーバー状態を確認する Playbook（15分）
    ↓
Step 4: サーバー再起動の Playbook（15分）
    ↓
Step 5: サービス管理の Playbook（10分）
    ↓
Step 6: 🔧 障害対応シミュレーション — EC2に障害発生！（25分）
    ↓
振り返り（10分）
```
