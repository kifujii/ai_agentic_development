...
1. **再起動前**: 稼働時間と重要サービス（sshd, chronyd）の状態が表示される
...
対象: webserversグループ
処理の流れ:
1. 再起動前: 稼働時間と重要サービス（sshd, chronyd）の状態を確認・表示
...
対象: webserversグループ
機能:
- 変数 target_service でサービス名を指定（デフォルト: chronyd）
- 変数 target_action でアクション指定（started/stopped/restarted）
- 変更前後のサービス状態を表示

作成後、chronyd を再起動するように実行してください。
```
...
対象: webserversグループ
処理の流れ:
1. 主要サービス（sshd, nginx, chronyd）の起動状態を確認
2. ディスク使用量のチェック
3. メモリ使用量のチェック
4. 最近のシステムログ確認（journalctl の直近エラー）
5. すべての結果をサマリーとして表示
...
private_key_file = ~/workspace/keys/training-key
...
ansible_ssh_private_key_file=~/workspace/keys/training-key
...
> 💡 このPlaybookは Phase 2 の調査・復旧で得た知見をもとに、**汎用的なサーバー診断ツール**として作成します。実際の障害パターンに応じて受講者ごとに内容が異なることが期待されます。以下は一般的な診断項目の例です。
...
- name: サーバー診断
  ...
  tasks:
    ...
    - name: エラーログの表示
      debug:
        msg: "{{ error_log.stdout_lines | default(['ログ取得不可']) }}"
    ...
    - name: サマリー
      debug:
        msg: |
          ...
          {% endfor %}
          ディスク・メモリ: 上記出力を参照
          直近エラー: {{ error_log.stdout_lines | default([]) | length }} 件
```
