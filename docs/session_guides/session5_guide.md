...
```
AWS CLI を使って、EC2 が SSM と CloudWatch にアクセスできる IAM ロールを作成し、
EC2 に関連付けてください。
- ロール名、プロファイル名はプレフィックス（$TF_VAR_prefix）付き
- 必要なポリシー: AmazonSSMManagedInstanceCore, CloudWatchAgentServerPolicy
- EC2 のインスタンスIDは terraform output で確認できます
```
