...
EC2 が CloudWatch と SSM に対して API 通信できる権限を持っている状態。IAM ロールとインスタンスプロファイルが作成・関連付けされ、後続の Step で SSM Agent や CloudWatch Agent をインストールすれば即座に動作する。
...
EC2 上で SSM Agent が稼働している状態。AWS Systems Manager からリモート管理が可能になり、次の Step で SSM 経由の接続テストができる。
...
EC2 に CloudWatch Agent がインストールされている状態。まだ設定・起動はしていないが、次の Step で設定を配置すればメトリクス収集を開始できる。
...
CloudWatch Agent が設定に基づいて CPU・メモリ・ディスクのメトリクスとログを収集・送信している状態。CloudWatch コンソールの `Training/EC2` 名前空間でメトリクスが確認できる。
...
CPU使用率が80%を超えると自動的にアラーム状態になる監視ルールが設定されている状態。CloudWatch コンソールでアラームの存在と状態を確認できる。

### やること

- CPU使用率が80%を超えたらアラーム状態になる CloudWatch Alarm を AWS CLI で作成する
