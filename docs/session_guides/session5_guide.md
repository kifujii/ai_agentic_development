...
- アタッチ済みポリシー: `CloudWatchAgentServerPolicy`, `AmazonSSMManagedInstanceCore`
- インスタンスプロファイル: `${PREFIX}-ec2-agent-profile`
- EC2 にプロファイルが関連付けられている

> ⚠️ IAMリソース名には必ず自分のPREFIXを含めてください。`$PREFIX` は環境変数として設定済みです（例: `echo $PREFIX` で確認）。
...
1. IAM ロール ${PREFIX}-ec2-agent-role を作成（$PREFIX は環境変数から取得して）
   - 信頼ポリシー: EC2 サービスからの AssumeRole を許可
2. ポリシーをアタッチ:
   - arn:aws:iam::aws:policy/CloudWatchAgentServerPolicy
   - arn:aws:iam::aws:policy/AmazonSSMManagedInstanceCore
3. インスタンスプロファイル ${PREFIX}-ec2-agent-profile を作成
4. ロールをインスタンスプロファイルに追加
5. terraform -chdir=terraform/vpc-ec2 output instance_id を実行してEC2のIDを取得
   （エラーの場合は terraform -chdir=terraform/vpc-ec2 output で全出力を確認し、インスタンスIDを探してください）
...
再度関連付け：
```bash
aws ec2 associate-iam-instance-profile --instance-id <ID> --iam-instance-profile Name=${PREFIX}-ec2-agent-profile
```
...
- 収集するメトリクス: CPU使用率、メモリ使用率、ディスク使用率
- 収集するログ: /var/log/messages, /var/log/secure
- メトリクス名前空間: ${PREFIX}/EC2（$PREFIX は環境変数から取得して）
- ロググループ名: /${PREFIX}/ec2/messages, /${PREFIX}/ec2/secure
...
1. **CloudWatch → メトリクス → カスタム名前空間 → ${PREFIX}/EC2** でメトリクスを確認
2. **CloudWatch → ロググループ → /${PREFIX}/ec2/** でログを確認
...
- アラーム名: ${PREFIX}-cpu-alarm（$PREFIX は環境変数から取得して）
- メトリクス: ${PREFIX}/EC2 名前空間の cpu_usage_user
- 条件: 1分間の平均が80%以上
- 比較期間: 1期間
- アクション: なし（通知は不要）
...
**AWS コンソール** → **CloudWatch → アラーム** で `${PREFIX}-cpu-alarm` が表示されていれば OK ✅
...
- name: CloudWatch Agent設定・起動
  ...
  vars:
    prefix: "{{ lookup('env', 'TF_VAR_prefix') }}"
    cwagent_config:
      agent:
...
```bash
aws iam remove-role-from-instance-profile --instance-profile-name ${PREFIX}-ec2-agent-profile --role-name ${PREFIX}-ec2-agent-role
```

ポリシーのデタッチ：
```bash
aws iam detach-role-policy --role-name ${PREFIX}-ec2-agent-role --policy-arn arn:aws:iam::aws:policy/CloudWatchAgentServerPolicy
```

```bash
aws iam detach-role-policy --role-name ${PREFIX}-ec2-agent-role --policy-arn arn:aws:iam::aws:policy/AmazonSSMManagedInstanceCore
```

リソース削除：
```bash
aws iam delete-instance-profile --instance-profile-name ${PREFIX}-ec2-agent-profile
```

```bash
aws iam delete-role --role-name ${PREFIX}-ec2-agent-role
```

> 💡 Claude Code に「`${PREFIX}-ec2-agent-role` と `${PREFIX}-ec2-agent-profile` を削除して」と伝えれば、上記コマンドを実行してくれます。

CloudWatch Alarm の削除：
```bash
aws cloudwatch delete-alarms --alarm-names ${PREFIX}-cpu-alarm
```

CloudWatch ロググループの削除（作成した場合のみ）：
```bash
aws logs delete-log-group --log-group-name /${PREFIX}/ec2/messages
```

```bash
aws logs delete-log-group --log-group-name /${PREFIX}/ec2/secure
```
