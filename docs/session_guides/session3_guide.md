...
2. moved ブロックを追加して、既存の EC2 が再作成されないようにする
   （aws_instance.training_ec2 → aws_instance.training_ec2[0] への移行）
3. Name タグを "${var.prefix}-ec2-1", "${var.prefix}-ec2-2" のように count.index で区別する
4. outputs.tf を複数台に対応させる：
   - instance_public_ip → instance_public_ips（リスト形式、splat 式 [*] を使用）
   - instance_id → instance_ids（リスト形式）
5. 既存の単数形の output も残して、1台目の値を返すようにする
   （他のセッションで参照しているため）
