...
| 7 | 未知の技術を AI で攻略する | 2h | 必須 | Terraform |
| 8 | 本番リリースの設計判断 | 2h | 必須 | Terraform |
| 9 | インシデント対応とポストモーテム | 1.5h | 必須 | Ansible + SSH |
| 10 | ゼロからシステム構築チャレンジ | 2h | 任意 | 全ツール |
...
- 必須合計: 14h15min / 任意合計: 3.75h / 全体: 18h（任意含む）
- ⏱️ 各セッションの時間にはバッファ（約10〜15分）を含んでいます。環境トラブルやAgent応答待ちに充ててください。
- 💡 Day 3 の Session 7-9 は各セッション独立しています。順番を入れ替えても問題ありません。
...
### セッション9：インシデント対応とポストモーテム（必須・1.5h）
...
### セッション10：ゼロからシステム構築チャレンジ（任意・2h）

社内ハッカソン形式で、3つのお題（ステータスページ / デプロイパイプライン / 複数環境管理）から1つを選んで2時間で動くものを作ります。
