...
Ansibleを使ってSSM AgentとCloudWatch Agentをインストール・設定します。**意図的にエラーが発生する手順**を含んでおり、Claude Code によるトラブルシューティング体験ができます。

**構築ステップ**:
1. SSM Agentインストール（Ansible）
2. SSM Agent動作確認 → 🔧 トラブルシューティング（IAMロール不足）
3. SSM Run Command 体験
4. CloudWatch Agentインストール（Ansible）
5. CloudWatch Agent設定・起動（Ansible）
6. CloudWatch確認 → 🔧 トラブルシューティング（名前空間不備）
7. CloudWatch Alarm 作成

**学ぶこと**: Agent によるトラブルシューティング、ツールの使い分け（Terraform vs Ansible vs AWS CLI vs SSM）
