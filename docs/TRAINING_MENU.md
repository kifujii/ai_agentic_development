...
| 2 | Webシステム構築 (ALB/ECS/RDS) | 1h | 任意 | Terraform |
...
- 必須合計: 4.5h / 任意合計: 2h / 全体: 7.5h（任意含む）
...
### セッション2：Webシステム構築（任意・1h）

セッション1のVPCを活用し、ALB + ECS/ECR + RDS を含むWebアプリケーションインフラを構築します。

**構築ステップ**:
1. サブネット追加（パブリック×2、プライベート×2）
2. ALB作成
3. ECS/ECR作成
4. RDS作成
5. 動作確認
