...
| 5 | SSM Agent & CloudWatch Agent 導入 | 55min | 必須 | Ansible + AWS CLI |
...
### セッション5：SSM Agent & CloudWatch Agent 導入（必須・55min）

EC2を AWS API 経由でリモート管理・監視できる状態にします。「何が必要か」から Claude Code と一緒に考えるチャレンジ形式です。

**チャレンジ**:
- 前半: SSM Agent を導入して Systems Manager からリモート管理できるようにする
- 後半: CloudWatch Agent で CPU/メモリ/ディスクの監視基盤を構築する
