...
| 2 | VPC/EC2の設計・構築・検証 | 1.5h | 必須 | Terraform | 基本AWSネットワーク+EC2が動作する環境 |
| 3 | Webシステム構築 (ALB/ECS/ECR/RDS) | 1h | 任意 | Terraform | Webアプリケーションインフラ一式 |
| 4 | サーバー再起動の自動化 | 1.5h | 必須 | Ansible | サーバー再起動Playbook+基本運用タスク |
| 5 | CloudWatch Agentインストール・セットアップ | 1.5h | 必須 | Terraform+Ansible | CloudWatch Agent導入済み環境 |
...
├── Session 2: VPC/EC2の設計・構築・検証 (1.5h)  [必須]
└── Session 3: Webシステム構築 (1h)                      [任意]

Day 2 (4h): システム運用 (Ansible)
├── Session 4: サーバー再起動の自動化 (1.5h)              [必須]
├── Session 5: CloudWatch Agentインストール・セットアップ (1.5h) [必須]
...
### セッション2：VPC/EC2の設計・構築・検証（必須）（1.5時間）

#### 目標
Agent形式でVPC/EC2を構築し、構築結果を検証する。セッション4以降でAnsibleから操作するためのEC2環境を完成させる。
...
### セッション5：CloudWatch Agentインストール・セットアップ（必須）（1.5時間）
...
Session 1: 基礎理解
    ↓
Session 2: VPC/EC2構築 ──→ Session 3: Webシステム構築（任意）
